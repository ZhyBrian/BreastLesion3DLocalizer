...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads=12, mlp_ratio=4.,
                 ...
                 last_fusion=False, num_med_block=0, groups=1, drop_path_rate_cnn=0.):

        super(ConvTransBlock, self).__init__()
        expansion = 4
        self.cnn_block = ConvBlock(inplanes=inplanes, outplanes=outplanes, res_conv=res_conv, stride=stride, groups=groups, 
                                   drop_path=drop_path_rate_cnn)

        if last_fusion:
            self.fusion_block = ConvBlock(inplanes=outplanes, outplanes=outplanes, stride=2, res_conv=True, groups=groups,
                                          drop_path=drop_path_rate_cnn)
        else:
            self.fusion_block = ConvBlock(inplanes=outplanes, outplanes=outplanes, groups=groups, drop_path=drop_path_rate_cnn)

        if num_med_block > 0:
            self.med_block = []
            for i in range(num_med_block):
                self.med_block.append(Med_ConvBlock(inplanes=outplanes, groups=groups, drop_path=drop_path_rate_cnn))
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, base_channel=64, channel_ratio=4, num_med_block=0,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.3, drop_path_rate_cnn=0.3):
        ...
        self.cnn_dpr = [x.item() for x in torch.linspace(0, drop_path_rate_cnn, depth)]  # stochastic depth decay rule
        ...
        self.conv_1 = ConvBlock(inplanes=64, outplanes=stage_1_channel, res_conv=True, stride=1, drop_path=self.cnn_dpr[0])
        ...
        for i in range(init_stage, fin_stage):
            self.add_module('conv_trans_' + str(i),
                    ConvTransBlock(
                        ...
                        num_med_block=num_med_block, drop_path_rate_cnn=self.cnn_dpr[i-1]
                    )
            )
        ...
        for i in range(init_stage, fin_stage):
            ...
            self.add_module('conv_trans_' + str(i),
                    ConvTransBlock(
                        ...
                        num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=self.trans_dpr[i-1],
                        num_med_block=num_med_block, last_fusion=last_fusion, drop_path_rate_cnn=self.cnn_dpr[i-1]
                    )
            )
...
class UNet_ConvFormer(nn.Module):
    def __init__(self,
        ...
        if size == 'small':
            self.seg_backbone = Conformer_small_patch16(pretrained=True)

            self.up1 = Up(base_c * 16, base_c * 8, bilinear, conv_num=3)
            self.up2 = Up(base_c * 8, base_c * 4, bilinear, conv_num=3)
            self.up3 = Up(base_c * 4, base_c * 1, bilinear, conv_num=2)
            self.up4 = Up(base_c * 1, base_c, bilinear, conv_num=2, skip=False)
            self.out_conv = OutConv(base_c, num_classes)
        elif size == 'base':
            self.seg_backbone = Conformer_base_patch16(pretrained=True)
        ...
        # if pretrained:
        #     misskeys, unexpectkeys = self.load_state_dict(torch.load(r''), strict=False)
        #     print("misskeys: ", *misskeys, sep='\n')
        #     print("unexpectkeys: ", *unexpectkeys, sep='\n')
        #     print('successful load weight for UNet_ConvFormer!')
...
class Classifier(nn.Module):
    def __init__(self, embed_dim, num_classes, 
                 iter_num_dilate=25, iter_num_shrink=15, drop_path=0.):
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.linear_final = BasicLinearBlock(embed_dim * 4, 768, dropoutrate=0.2)
    ...
    def forward(self, x_t, mask=None):
        ...
        x_t = x_t + self.drop_path(self.mtr_attn(self.norm1(x_t), mask))
        
        x_t = self.trans_norm(x_t)
        # print(x_t.shape)
        # [B, 256, 384] -> [B, 384, 256] -> [B, 384, 16, 16]
        ...
        # print(x_r.shape)
        # print(torch.flatten(self.avgpool(x_r), 1).shape)
...
class MultiRegionAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            # print(self.iter_num_dilate, self.iter_num_shrink)
...
def Conformer_tiny_patch16(pretrained=False, **kwargs):
    ...
    if pretrained:
        misskeys, unexpectkeys = model.load_state_dict(torch.load(r'model_data\Conformer_tiny_patch16.pth'), strict=False)
        print("misskeys: ", *misskeys, sep='\n')
        print("unexpectkeys: ", *unexpectkeys, sep='\n')
...
def Conformer_base_patch16(pretrained=False, **kwargs):
    ...
    if pretrained:
        misskeys, unexpectkeys = model.load_state_dict(torch.load(r'model_data\Conformer_base_patch16.pth'), strict=False)
        print("misskeys: ", *misskeys, sep='\n')
        print("unexpectkeys: ", *unexpectkeys, sep='\n')
        print('successful load weight for Conformer backbone!')
        # raise NotImplementedError
...
if __name__ == '__main__':
    ...
    torch.save(net.state_dict(), f'params/Conformer_MTL.pth')   #
    print('parameters_count:', count_parameters(net) / 1000**2, 'M')
